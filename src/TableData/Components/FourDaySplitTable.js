...
export default function FourDaySplitTable() {
  ...
  const colRef = collection(db, "FourDay");

  const getRef = query(
    collection(db, "FourDay"),
    where("uid", "==", user),
    orderBy("date")
  );

  const getFourDaySplit = async () => {
    let data = [];
  ...
  useEffect(() => {
    ...
    const docRef = doc(db, "FourDay", `${toDelete}`);
    deleteDoc(docRef)
      .then(() => {})
      .catch(() => {});
  }, [toDelete]);
  ...
  return (
    <div className="table-wrapper">
      ...
      {showButtons ? (
        <Buttons adds={handleSubmit} getData={getFourDaySplit} />
      ) : null}
