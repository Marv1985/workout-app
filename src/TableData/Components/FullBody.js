...
export default function FullBody() {
  ...
  const colRef = collection(db, "FullBody");

  const getRef = query(
    collection(db, "FullBody"),
    where("uid", "==", user),
    orderBy("date")
  );

  const getFullBodySplit = async () => {
    let data = [];
  ...
  useEffect(() => {
    ...
    const docRef = doc(db, "FullBody", `${toDelete}`);
    deleteDoc(docRef)
      .then(() => {})
      .catch(() => {});
  }, [toDelete]);
  ...
  return (
    <div className="table-wrapper">
      ...
      {showButtons ? (
        <Buttons adds={handleSubmit} getData={getFullBodySplit} />
      ) : null}
